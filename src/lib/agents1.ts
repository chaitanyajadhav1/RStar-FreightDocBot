// src/lib/agent.ts - COMPLETE OPENAI-POWERED INVOICE EXTRACTION SYSTEM
...
// ============================================
// OPENAI-POWERED EXTRACTION FUNCTIONS
// ============================================
const openai = new OpenAIClient(OPENAI_CONFIG);

async function extractBasicInfoWithOpenAI(invoiceText: string): Promise<{
  invoiceNo: string | null;
  marksandnos:string |null;
  date: string | null;
  referenceNo: string | null;
  ...
  console.log('[OpenAI] Extracting basic invoice information...');
...
For totalAmount, return just the number without commas.`;
  ...
  if (!result || !result.invoiceNo || !result.date) {
    console.log('[OpenAI] Retrying basic info extraction with simplified prompt...');
...
Return as JSON with keys: invoiceNo,marksandnos date, totalAmount, currency.`;
  ...
  if (result) {
    console.log('[OpenAI] Basic info extracted:', {
...
async function extractItemsWithOpenAI(invoiceText: string): Promise<Array<{
  description: string;
  quantity: string;
  unitPrice: number;
  totalPrice: number;
}>> {
  console.log('[OpenAI] Extracting invoice items...');
...
Extract ALL items, do not skip any rows.`;
  ...
  if (result && result.items && Array.isArray(result.items) && result.items.length > 0) {
    console.log(`[OpenAI] Extracted ${result.items.length} items from invoice`);
    ...
    if (cleanedItems.length < 7) {
      console.warn(`[OpenAI] Warning: Only extracted ${cleanedItems.length} items, expected 7. Trying alternative extraction...`);
...
Return all 7 items as JSON array.`;
      ...
      if (retryResult && retryResult.items && retryResult.items.length > cleanedItems.length) {
        console.log(`[OpenAI] Retry successful: Found ${retryResult.items.length} items`);
  ...
  console.log('[OpenAI] No items extracted');
  return [];
}

async function extractExporterWithOpenAI(invoiceText: string): Promise<{
  name: string | null;
  address: string | null;
  factory: string | null;
  pan: string | null;
  ...
  console.log('[OpenAI] Extracting exporter details...');
...
Return ONLY valid JSON.`;
  ...
  if (result) {
    console.log('[OpenAI] Exporter extracted:', result.name);
    return {
...
async function extractConsigneeWithOpenAI(invoiceText: string): Promise<{
  name: string | null;
  address: string | null;
  phone: string | null;
  mobile: string | null;
  ...
  console.log('[OpenAI] Extracting consignee details...');
...
Extract all available contact details.`;
  ...
  if (result) {
    console.log('[OpenAI] Consignee extracted:', result.name);
    return {
      name: result.name || null,
      address: result.address || null,
      phone: result.phone || null,
...
async function extractBankWithOpenAI(invoiceText: string): Promise<{
  bankName: string | null;
  address: string | null;
  usdAccount: string | null;
  euroAccount: string | null;
  ...
  console.log('[OpenAI] Extracting bank details...');
...
Extract account numbers, codes, and bank address.`;
  ...
  if (result) {
    console.log('[OpenAI] Bank extracted:', result.bankName);
    return {
...
async function extractShippingWithOpenAI(invoiceText: string): Promise<{
  incoterms: string | null;
  preCarriage: string | null;
  placeOfReceipt: string | null;
  vesselFlight: string | null;
  ...
  console.log('[OpenAI] Extracting shipping details...');
...
Look for information about ports, destinations, shipping terms, and HSN codes.`;
  ...
  if (result) {
    console.log('[OpenAI] Shipping extracted');
...
async function extractAdditionalInfoWithOpenAI(invoiceText: string): Promise<{
  paymentTerms: string | null;
  marksAndNumbers: string | null;
  packaging: string | null;
  igstStatus: string | null;
  ...
  console.log('[OpenAI] Extracting additional information...');
  ...
  if (result) {
    console.log('[OpenAI] Additional info extracted');
    return {
...
export async function extractAndValidateInvoice(invoiceText: string): Promise<InvoiceValidationResult> {
  console.log('═══════════════════════════════════════');
  console.log('[Extraction] Starting OpenAI-powered invoice extraction');
  ...
  try {
    ...
    const [basicInfo, items, exporter, consignee, bank, shipping, additional] = await Promise.all([
      extractBasicInfoWithOpenAI(invoiceText),
      extractItemsWithOpenAI(invoiceText),
      extractExporterWithOpenAI(invoiceText),
      extractConsigneeWithOpenAI(invoiceText),
      extractBankWithOpenAI(invoiceText),
      extractShippingWithOpenAI(invoiceText),
      extractAdditionalInfoWithOpenAI(invoiceText)
    ]);
...
export async function testOpenAIConnection(): Promise<{ success: boolean; message: string; model: string }> {
  try {
    ...
    const testResponse = await openai.generate('Respond with just "OK"', 'You are a test assistant');
...
async function extractExportDeclarationBasicInfo(invoiceText: string): Promise<{
  ...
  console.log('[OpenAI] Extracting export declaration basic information...');
...
This is typically a customs declaration form, not a commercial invoice.`;
  ...
  if (result) {
    console.log('[OpenAI] Export declaration basic info extracted:', {
      invoiceNo: result.invoiceNo,
...
async function extractValuationDetails(invoiceText: string): Promise<{
  ...
  console.log('[OpenAI] Extracting valuation details...');
...
async function extractTransactionDetails(invoiceText: string): Promise<{
  ...
  console.log('[OpenAI] Extracting transaction details...');
...
async function extractDeclarationStatus(invoiceText: string): Promise<{
  ...
  console.log('[OpenAI] Extracting declaration status...');
...
export async function extractAndValidateExportDeclaration(invoiceText: string): Promise<ExportDeclarationValidationResult> {
  console.log('═══════════════════════════════════════');
  console.log('[Extraction] Starting OpenAI-powered export declaration extraction');
  console.log('[Extraction] Text length:', invoiceText.length);
  console.log('═══════════════════════════════════════');
...
async function extractSCOMETBasicInfo(documentText: string): Promise<{
  ...
  console.log('[OpenAI] Extracting SCOMET basic information...');
...
Return ONLY valid JSON.`;
  ...
  if (result) {
    console.log('[OpenAI] SCOMET basic info extracted');
    return {
...
async function extractSCOMETReferenceInfo(documentText: string): Promise<{
  ...
  console.log('[OpenAI] Extracting SCOMET reference information...');
...
Return ONLY valid JSON.`;
  ...
  if (result) {
    console.log('[OpenAI] SCOMET reference info extracted:', {
...
async function extractSCOMETGoodsInfo(documentText: string): Promise<{
  ...
  console.log('[OpenAI] Extracting SCOMET goods information...');
...
Return ONLY valid JSON.`;
  ...
  if (result) {
    console.log('[OpenAI] SCOMET goods info extracted:', {
...
async function extractSCOMETSignatureInfo(documentText: string): Promise<{
  ...
  console.log('[OpenAI] Extracting SCOMET signature information...');
...
Return ONLY valid JSON.`;
  ...
  if (result) {
    ...
    console.log('[OpenAI] SCOMET signature extracted:', {
      signed: signedStatus,
...
export async function extractAndValidateSCOMETDeclaration(documentText: string): Promise<SCOMETDeclarationValidationResult> {
  console.log('═══════════════════════════════════════');
  console.log('[Extraction] Starting OpenAI-powered SCOMET declaration extraction');
...
async function extractPackingListBasicInfo(documentText: string): Promise<{
  ...
  console.log('[OpenAI] Extracting packing list basic information...');
...
Return ONLY valid JSON with the extracted values or null if not found.`;
  ...
  if (result) {
    console.log('[OpenAI] Packing list basic info extracted:', {
...
async function extractPackingListExporter(documentText: string): Promise<ExporterInfo> {
  console.log('[OpenAI] Extracting exporter information...');
...
async function extractPackingListConsignee(documentText: string): Promise<ConsigneeInfo> {
  console.log('[OpenAI] Extracting consignee information...');
...
Return ONLY valid JSON.`;
  ...
  if (result) {
    console.log('[OpenAI] Bank details extracted:', result.bankName);
    return {
      bankName: result.bankName || null,
...
async function extractPackingListShippingInfo(documentText: string): Promise<ShipmentDetails> {
  console.log('[OpenAI] Extracting shipping information...');
...
Return ONLY valid JSON.`;
  ...
  if (result) {
    console.log('[OpenAI] Shipping info extracted');
...
async function extractPackingListBoxDetails(documentText: string): Promise<{
  ...
  console.log('[OpenAI] Extracting box details...');
...
Return ONLY valid JSON with an array of all boxes found.`;
  ...
  if (result && result.boxDetails && Array.isArray(result.boxDetails)) {
    console.log('[OpenAI] Box details extracted:', result.boxDetails.length, 'boxes');
  ...
  console.log('[OpenAI] No box details found');
  return {
    boxDetails: [],
...
export async function extractAndValidatePackingList(documentText: string): Promise<PackingListValidationResult> {
  console.log('═══════════════════════════════════════');
  console.log('[Extraction] Starting OpenAI-powered packing list extraction');
  console.log('[Extraction] Text length:', documentText.length);
  console.log('[Extraction] Using model:', OPENAI_CONFIG.model);
  console.log('═══════════════════════════════════════');
...
async function extractFumigationCertificateBasicInfo(documentText: string): Promise<{
  ...
  console.log('[OpenAI] Extracting fumigation certificate basic information...');
...
Return ONLY valid JSON.`;
  ...
  if (result) {
    console.log('[OpenAI] Fumigation certificate basic info extracted:', {
...
async function extractFumigationTreatmentDetails(documentText: string): Promise<{
  ...
  console.log('[OpenAI] Extracting fumigation treatment details...');
...
Return ONLY valid JSON.`;
  ...
  if (result) {
    console.log('[OpenAI] Treatment details extracted:', {
...
async function extractFumigationCargoDetails(documentText: string): Promise<{
  ...
  console.log('[OpenAI] Extracting fumigation cargo details...');
...
Return ONLY valid JSON.`;
  ...
  if (result) {
    console.log('[OpenAI] Cargo details extracted:', {
...
async function extractFumigationOperatorInfo(documentText: string): Promise<{
  ...
  console.log('[OpenAI] Extracting fumigation operator information...');
...
Return ONLY valid JSON.`;
  ...
  if (result) {
    ...
    console.log('[OpenAI] Operator info extracted:', {
 ...
 * Uses OpenAI AI to extract all relevant information and validates completeness
 */
export async function extractAndValidateFumigationCertificate(
  documentText: string
): Promise<FumigationCertificateValidationResult> {
  console.log('═══════════════════════════════════════');
  console.log('[Extraction] Starting OpenAI-powered fumigation certificate extraction');
  console.log('[Extraction] Text length:', documentText.length);
  console.log('[Extraction] Using model:', OPENAI_CONFIG.model);
  console.log('═══════════════════════════════════════');
...
export async function extractAndValidateAirwayBill(documentText: string): Promise<AirwayBillValidationResult> {
  console.log('═══════════════════════════════════════');
  console.log('[Extraction] Starting OpenAI-powered airway bill extraction');
  console.log('[Extraction] Text length:', documentText.length);
  console.log('[Extraction] Model:', OPENAI_CONFIG.model);
  console.log('═══════════════════════════════════════');
